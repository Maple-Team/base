...
export const envVariables = (dirname: string, options?: Options): Record<string, string> => {
  ...
  if (fs.existsSync(localPath)) localEnvKeys = getEnvKeys(localPath, options)

  if (fs.existsSync(envPath)) modeEnvKeys = getEnvKeys(envPath, options)

  if (fs.existsSync(basePath)) baseEnvKeys = getEnvKeys(basePath, options)
