...
const calendar = {
   ...
   */
  solar2lunar(y1?: number, m1?: number, d1?: number) {
    if (!y1 || !m1 || !d1) throw new Error('年月日参数必需为数字')

    // 参数区间1900.1.31~2100.12.31
    // 年份限定、上限
    if (y1 < 1900 || y1 > 2100) throw new Error('年份越界')

    // 公历传参最下限
    if (y1 === 1900 && m1 === 1 && d1 < 31) throw new Error('年份日期越界')

    // 未传参  获得当天
    let objDate
    if (!y1) objDate = new Date()
    else objDate = new Date(y1, m1 - 1, d1)
    ...
    if (d >= firstNode) gzM = calendar.toGanZhi((y - 1900) * 12 + m + 12)
   ...
   */
  lunar2solar(y: number, m: number, d: number, isLeapMonth1: boolean) {
    // 参数区间1900.1.31~2100.12.1
    const isLeapMonth = !!isLeapMonth1
    // const leapOffset = 0
    ...
    if (isLeapMonth) _day = calendar.leapDays(y)
    ...
    // 计算农历的时间差
    let offset = 0
    for (let i = 1900; i < y; i++) offset += calendar.lYearDays(i)
