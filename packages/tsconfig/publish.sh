...
# Main gets published as LATEST if that version doesn't exists yet and retagged as LATEST otherwise.
if [ "${BRANCH}" = "master" ]; then
    ...
    printf "${RED}@liutsing/tsconfig package uses only a main branch which gets published with a latest NPM tag!${NC}\n"
    exit 1
fi
