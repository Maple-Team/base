import { hash, isHans, transformKey, transformKeyWithoutHash } from '../src/helper'
...
describe('auto-i18n plugin help method', () => {
  ...
  it('isHans', () => {
    expect(isHans('中')).toBe(true)
    expect(isHans('中a')).toBe(true)
    expect(isHans('中a中')).toBe(true)
    expect(isHans('中a中a')).toBe(true)
    expect(isHans('abc')).toBe(false)
    expect(isHans('123')).toBe(false)
  })
})
